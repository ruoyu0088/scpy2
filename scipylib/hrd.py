...
def get_moves(node):
    _Moves = Moves
    block_moves = [_Moves[c] for c in "ABBBBCDDDD"]
...
def dump_graph():
    ...
    with file("hrd.pickle", "wb") as f:
        cPickle.dump({"nodes": compressed_nodes, "edges": edges, "moves": moves}, f)


def load_graph():
    with file("hrd.pickle", "rb") as f:
...
if __name__ == '__main__':
    dump_graph()
    graph = load_graph()
    find_path(graph, "BAABBAABBCCBBDDBD  D")
    #find_path(graph, "BAABBAABDCCDBDDBB  B")