...
from scpy2.traitslib import MPLFigureEditor, AskName, SettingManager
...
class IFSDesigner(HasTraits):
    ...
    view = View(
        VGroup(
            HGroup(
                Item("add_button"),
                Item("del_button"),
                Item("current_name", editor=EnumEditor(name="ifs_names")),
                Item("save_button"),
                Item("unsave_button"),
    ...
    def _current_name_changed(self):
        ifs = self.ifs_list[self.ifs_names.index(self.current_name)]
        self.ifs_triangle.set_points(np.array(ifs["points"]))
        self.current_map = ifs["cmap"]
    ...
    def _save_button_fired(self):
        ...
        if ask.configure_traits():
            data = dict(name=ask.name,
                        points=self.ifs_triangle.points.tolist(),
                        cmap=self.current_map)
            if ask.name not in self.ifs_names:
                self.ifs_names.append(ask.name)
                self.ifs_list.append(data)
            else:
                index = self.ifs_names.index(ask.name)
                self.ifs_names[index] = ask.name
                self.ifs_list[index] = data
            self.save_data()

    def save_data(self):
        with file(DATA_FILE, "wb") as f:
            json.dump(self.ifs_list, f)
    ...
    def init_gui_component(self):
        ...
        if path.exists(DATA_FILE):
            with file(DATA_FILE, "rb") as f:
                self.ifs_list = json.load(f)
                self.ifs_names = [x["name"] for x in self.ifs_list]
